...
fn crud_set_and_get() {
    ...
    engine
        .set(&BAG.to_string(), &"key".to_string(), b"hello world")
        .unwrap();
...
fn crud_exists_false_for_missing_key() {
    ...
    assert!(
        !engine
            .exists(&BAG.to_string(), &"ghost".to_string())
            .unwrap()
    );
}
...
fn crud_exists_false_for_missing_bag() {
    ...
    assert!(
        !engine
            .exists(&"no-bag".to_string(), &"k".to_string())
            .unwrap_or(false)
    );
}
...
fn crud_list_keys() {
    let (_dir, engine) = new_engine_with_bag(BAG);
    engine
        .set(&BAG.to_string(), &"k1".to_string(), b"v")
        .unwrap();
    engine
        .set(&BAG.to_string(), &"k2".to_string(), b"v")
        .unwrap();
    engine
        .set(&BAG.to_string(), &"k3".to_string(), b"v")
        .unwrap();
...
fn crud_multiple_bags_are_isolated() {
    ...
    engine
        .set(&"a".to_string(), &"k".to_string(), b"from-a")
        .unwrap();
    engine
        .set(&"b".to_string(), &"k".to_string(), b"from-b")
        .unwrap();

    assert_eq!(
        engine.get(&"a".to_string(), &"k".to_string()).unwrap(),
        b"from-a"
    );
    assert_eq!(
        engine.get(&"b".to_string(), &"k".to_string()).unwrap(),
        b"from-b"
    );
...
fn edge_large_value() {
    ...
    engine
        .set(&BAG.to_string(), &"big".to_string(), &large)
        .unwrap();
...
fn edge_all_byte_values_roundtrip() {
    ...
    engine
        .set(&BAG.to_string(), &"bin".to_string(), &all_bytes)
        .unwrap();
